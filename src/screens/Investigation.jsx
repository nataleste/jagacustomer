...
/* -------------------------------------------------------------------------
   Static placeholder data. This is the shape the live SSE stream will fill
   later: each agent finding is { id, title, status, finding, sandbox? }.
   status drives the verdict coloring — "warning" (scam-red) or "checking"
   (amber, in-progress). Neutral/safe states slot in the same way.
   ------------------------------------------------------------------------- */
const AGENTS = [
  {
    id: 'script',
...
/* The fake-site preview rendered inside the Link card — the Daytona
   "sealed test space" money shot. DBS blue is a third-party brand color,
   intentionally outside the token system. */
function SandboxThumbnail({ url }) {
  return (
    <div className="mt-0.5 flex flex-col overflow-hidden rounded-chip border border-scam-border">
      <div className="flex items-center gap-[7px] bg-ink px-[10px] py-[7px]">
        ...
        <span className="flex-1 font-mono text-[11px] font-semibold leading-[14px] text-faint">
          {url}
        </span>
      ...
      <div className="flex flex-col gap-2 bg-[#F4F6F9] p-3.5">
        <span className="text-[16px] font-black leading-[18px] tracking-[0.04em] text-[#1B4DA0]">
          DBS
        </span>
...
function AgentCard({ Icon, title, status, finding, sandbox }) {
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-[5px]">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-[5px]">
            <span
              className={`h-2 w-2 rounded-full ${warning ? 'bg-scam' : 'bg-unsure-strong'}`}
            />
            <span
              className={`text-[13px] font-black leading-4 ${
                warning ? 'text-scam-ink' : 'text-unsure-ink'
              }`}
            >
        ...
        <p
          className={`text-[15px] font-medium leading-[21px] ${
            warning ? 'text-scam-deep' : 'text-subtle'
          }`}
        >
          {finding}
        </p>

        {sandbox && <SandboxThumbnail url={sandbox.url} />}
      </div>
    </div>
  )
}
      ...
      <div className="flex flex-1 flex-col gap-[18px] px-5 pb-5 pt-2">
        ...
        <header className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="whitespace-nowrap text-[16px] font-black leading-[18px] text-ink">
              0:08
            </span>
          </div>
        ...
        {/* Risk meter — climbs as findings land. Tap to reveal the verdict
            (mirrors the swarm completing). */}
        <Link
          to="/verdict/scam"
        ...
        {/* Spacer pins the footer to the bottom */}
        <div className="flex-1" />

        {/* Footer: privacy line + language toggle */}
        <div className="flex shrink-0 flex-col gap-[14px]">
          ...
          <div className="flex items-center justify-center gap-2">
            <button className="rounded-full bg-ink px-4 py-2 text-[14px] font-black leading-[18px] text-white">
              EN
            </button>
            <button className="rounded-full bg-fill px-4 py-2 text-[14px] font-black leading-[18px] text-subtle">
              中文
            </button>
            <button className="rounded-full bg-fill px-4 py-2 text-[14px] font-black leading-[18px] text-subtle">
              Melayu
